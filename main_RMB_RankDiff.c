...
int compare_complex(const void* a, const void* b) {
  double real_a = lapack_complex_double_real(*(lapack_complex_double*)a);
  double real_b = lapack_complex_double_real(*(lapack_complex_double*)b);

  // Sort in non-ascending order
  return (real_a < real_b) - (real_a > real_b);
}

// Generate a random vector of length 'size'
void func_random_vector(double *vec, int size)
{
  struct timespec ts;
...
// Generate a random density matrix of rank 'rank' and dimension 'dim'
lapack_complex_double* func_random_rho_fixed_rank(int dim, int rank)
{
  // Allocate memory for rho
  lapack_complex_double *rho = calloc(dim * dim, sizeof(lapack_complex_double));

  // Step 1: Generate diagonal matrix D of size 'rank'
  lapack_complex_double *D = calloc(rank, sizeof(lapack_complex_double));
  double *D_real = malloc(rank * sizeof(double)); // temporary array for real parts
  func_random_vector(D_real, rank); // populate D_real

  // Transfer real parts to D, set imaginary parts to zero
  for (int j = 0; j < rank; ++j) {
    D[j] = lapack_make_complex_double(D_real[j], 0.0);
  }

  free(D_real);  // Free temporary array

  // Sort D in non-ascending order
  qsort(D, rank, sizeof(lapack_complex_double), compare_complex);

  // Step 2: Generate random matrix U of size 'dim x rank'
  lapack_complex_double *U = calloc(dim * rank, sizeof(lapack_complex_double));
  for(int j = 0; j < rank; j++){
    for(int i = 0; i < dim; i++){
      double re = rand() / (double) RAND_MAX;
      double im = rand() / (double) RAND_MAX;
      // printf(" re = %lf, im = %lf \n", re, im);
      U[i + j * dim] = lapack_make_complex_double(re, im);
    }
  }

  // Orthonormalize U using QR decomposition
  lapack_complex_double *tau = calloc(rank * rank, sizeof(lapack_complex_double));

  // Perform QR decomposition to get Q and R
  LAPACKE_zgeqrf(LAPACK_COL_MAJOR, dim, rank, U, dim, tau);

  // Generate Q from the result of zgeqrf
  LAPACKE_zungqr(LAPACK_COL_MAJOR, dim, rank, rank, U, dim, tau);

  // Make a copy of U
  lapack_complex_double *U_copy = malloc(dim * rank * sizeof(lapack_complex_double));
  memcpy(U_copy, U, dim * rank * sizeof(lapack_complex_double));

  // Step 3: Create density matrix rho = U D U*
  // Intermediate matrix U * D
  for(int j = 0; j < rank; j++){
    for(int i = 0; i < dim; i++){
      U[i + j * dim] = U[i + j * dim] * D[j];
    }
  }
  // rho = U * U_copy*
  cblas_zgemm(CblasColMajor, CblasNoTrans, CblasConjTrans, dim, dim, rank, &one, U, dim, U_copy, dim, &zero, rho, dim);
...
int main(int argc, char* argv[])
{
  ...
  // Initialize states and uniform prior
  int states_per_rank = L / d;  // number of states for each rank
  int current_index = 0;        // index to keep track of where to place the next state

  for (int r = 1; r <= d; r++) {
      for (int n = 0; n < states_per_rank; n++) {
          mixed_states[current_index] = func_random_rho_fixed_rank(d, r);
          current_index++;
        }
  }
  ...
  for (int n = 0; n < N0; n++) {
    ...
    // double fid_test = fidelity_pure_state_density_matrix(d, &B[1*d], mixed_states[0]);
    // printf("Testing fidelity_pure_state_density_matrix(). fid_test = %lf.\n", fid_test);
  ...
  char filename[256];
  snprintf(filename, sizeof(filename), "plots_RMB/RankDiff/data_%d/RankDiff_average_risk_d=%d_N=%d_L=%d.txt", I0, d, N0, L);
