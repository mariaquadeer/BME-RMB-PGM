...
int main(int argc, char* argv[])
{
  ...
  I0 = atoi(argv[4]); /* output files in data_I0 folder */
   ...
   lapack_complex_double* pure_states[L];
   double prior[L];
   ...
   for (int a = 0; a < L; a++) {
     pure_states[a] = func_Haar_random_state();
     prior[a] = 1.0 / L;
   }
   ...
   for (int n = 0; n < N0; n++) {
     if (n > 0) { // Free B from the previous iteration
       free(B);
       }
    ...
    for (int x = 0; x < d; x++) {
      p_x[x] = 0;
      for (int a = 0; a < L; a++) {
        conditional_distribution[a][x] = cabs(cblas_zdotc(d, pure_states[a], 1, &B[x * d], 1));
        conditional_distribution[a][x] *= conditional_distribution[a][x];
  ...
  for (int a = 0; a < L; a++) {
    ...
    cblas_zgemm(CblasColMajor, CblasNoTrans, CblasConjTrans, d, d, 1, &temp, pure_states[a], d, pure_states[a], d, &one, Bayes_est, d);
  }
  ...
  // Evaluate risk of Bayes_est for pure_states[0] as a running sum over x
    double fid_0 = fidelity_pure_state_density_matrix(d, pure_states[0], Bayes_est);
    risk += conditional_distribution[0][x] * (fid_0);

  // Evaluate average_risk as running sum
    for (int a = 0; a < L; a++) {
    double fid = fidelity_pure_state_density_matrix(d, pure_states[a], Bayes_est);
  ...
  char filename[256];
  snprintf(filename, sizeof(filename), "plots_RMB/Pure/data_%d/Pure_average_risk_d=%d_N=%d_L=%d.txt", I0, d, N0, L);
